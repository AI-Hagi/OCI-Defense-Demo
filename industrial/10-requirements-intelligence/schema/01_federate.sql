...
-- SQLcl substitution off — the &OCI_* placeholders below are documented
-- defaults that ARE NOT runtime-resolved on Instance Principal'd dev VMs.
-- The credential and external-table BEGIN/END blocks fail-soft at runtime
-- (their EXCEPTION handlers swallow the lookup error). The programs
-- table at section 1c always succeeds, which is what 02_performance.sql
-- depends on.
SET DEFINE OFF
