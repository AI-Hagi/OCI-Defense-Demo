"""Offline test fixtures for flights-proxy."""
...
def mock_db(monkeypatch: pytest.MonkeyPatch) -> Any:
    ...
    state.audit_rows = []
    state.cache_rows = []
    state.classifier_rows: dict[str, tuple[str, str]] = {}
    ...
    def _execute(sql: str, params: Any = None) -> None:
        ...
        elif "MIL_AIRCRAFT_UNIFIED" in sql_upper and "SELECT" in sql_upper:
            hex24 = (params or {}).get("hex24") if isinstance(params, dict) else None
            row = state.classifier_rows.get(hex24)
            cursor.fetchone.return_value = row  # tuple (label, source) or None
