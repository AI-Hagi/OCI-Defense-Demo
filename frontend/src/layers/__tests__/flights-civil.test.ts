// Smoke tests for the UC4 Flights (Civil) layer module (Pattern A, ADR-0001).
//
...
describe('flights-civil layer module', () => {
  it('registers itself in LayerRegistry on import', () => {
    const layer = LayerRegistry.get('flights-civil');
    expect(layer).toBeDefined();
    expect(layer?.name).toBe('flights-civil');
  });
  ...
  it('label is human-readable and references civil', () => {
    const layer = LayerRegistry.get('flights-civil')!;
    expect(layer.label).toBeTruthy();
    expect(layer.label.toLowerCase()).toContain('civil');
  });
});
