...
- **Bbox-Default ist mehrfach referenziert.** Die Ostsee-Bbox `53,8,56,22` taucht in `services/ais-multiplexer/.env.example` (`AIS_BBOX_DEFAULT`), `app/settings.py` (Default-Property `bbox_default_tuple()`), und `db/schema/09_vessels_seed.sql` (Vessel-Koordinaten) auf. Drift ist möglich. **Empfehlung:** Single Source of Truth in `app/settings.py` definieren, andere Stellen importieren oder validieren gegen die Settings.
