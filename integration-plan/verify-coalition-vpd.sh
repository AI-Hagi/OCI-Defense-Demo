...
run_query() {
  ...
  sql -L "$DB_APP_USER/$DB_APP_PWD@$ADB_TNS_ALIAS" -nohistory <<SQL 2>/dev/null \
...
test_uc07() {
  ...
  MALLORY=$(sql -L "$DB_APP_USER/$DB_APP_PWD@$ADB_TNS_ALIAS" -nohistory <<'SQL' 2>/dev/null \
      | awk '/^[[:space:]]*[0-9]+[[:space:]]*$/ {print $1; exit}'
SET HEADING OFF
SET FEEDBACK OFF
SET PAGESIZE 0
...
# coalition_ctx_pkg.set_program(...) is a UC10-specific helper that
# scopes a session to one or more programs (e.g. 'EUROFIGHTER', 'FCAS').
# ---------------------------------------------------------------------
test_uc10() {
  ...
  ALICE=$(run_query "alice" "RESTRICTED" "DEU" "NATO" "requirements" \
    "EXEC coalition_ctx_pkg.set_program('EUROFIGHTER');")
  ...
  BOB=$(run_query "bob" "RESTRICTED" "FRA" "EU" "requirements" \
    "EXEC coalition_ctx_pkg.set_program('FCAS');")
  ...
  CAROL=$(run_query "carol" "RESTRICTED" "DEU" "NATO" "requirements" \
    "EXEC coalition_ctx_pkg.set_program('EUROFIGHTER,FCAS');")
